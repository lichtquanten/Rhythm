...
def backtest(algo_config):
    def handle_data(context, data):
        ...
        if algo['condition']['type'] == 'stocky':
            ...
            # print(str(diff))
            for stock in payload['stocks']:
                if stock['threshold_type'] == 'percent':
                    diff = diff/values[1]*100
                if stock['threshold'] > 0:
                    should_act = diff > stock['threshold']
                else:
                    should_act = diff < stock['threshold']
...
if __name__ == "__main__":
    ...
    backtest(nlp.splitString("If MSFT falls by $2 from yesterday AND TSLA gains 2% from purchase, buy 10 shares of AAPL"))
